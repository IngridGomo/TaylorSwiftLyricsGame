...
def the_game():
    ...
    tk.Label(window, text="Taylor Swift Lyrcis Game", font=("Times", 24)).place(
        x=215, y=10
    )
    tk.Button(window, text="Quick game", width=15, bg="pink", fg="black").place(
        x=130, y=70
    )
    tk.Button(window, text="Competition mode", bg="pink", fg="black").place(x=380, y=70)
    window.mainloop()
    ...
    while True:
        ...
        word_guess = str(
            input(
                "Which words are missing? Write them down with a space between, then press enter. "
            )
        )
